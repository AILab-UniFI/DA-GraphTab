...
from graphs.utils import center, distance, normalize
...
class GraphBuilder:
    ...
    """
    def __init__(self, config = CONFIG / 'graph.yaml'):
        
        with open(config) as fileobj:
            self.config = AttrDict(yaml.safe_load(fileobj))
      
    def print_graph(self, g, annotations, bboxs, page_path, out_path, labels=None, node_labels=True):
        ...
        if node_labels:
            ...
            for orig in annotations:
                ...
                draw.rectangle([(bbox[0]/UNIT_X, bbox[1]/UNIT_Y), (bbox[2]/UNIT_X, bbox[3]/UNIT_Y)], outline= color, width=3)
                
            # printing nodes
            for b, bbox in enumerate(bboxs):
                label = int(labels[b])
                color = (self.categories[label]['color'][0], self.categories[label]['color'][1], self.categories[label]['color'][2])
                draw.rectangle([(bbox[0]/UNIT_X, bbox[1]/UNIT_Y), (bbox[2]/UNIT_X, bbox[3]/UNIT_Y)], fill = color + (64,), outline=color, width=3)
        ...
        for e in range(0, len(start_edges)):
            ...
            if start_bb and end_bb:
                ...
                if [end_edges[e], start_edges[e]] in [[start_edges[o], end_edges[o]] for o in range(0, len(start_edges))]:
                    draw.line((int(center_start[0])/UNIT_X, int(center_start[1])/UNIT_Y, int(center_end[0])/UNIT_X, int(center_end[1])/UNIT_Y), fill=(128,91,0), width=3)
                else:
                    draw.line((int(center_start[0])/UNIT_X, int(center_start[1])/UNIT_Y, int(middle_point[0])/UNIT_X, int(middle_point[1])/UNIT_Y), fill=(0,255,0), width=3)
                    draw.line((int(middle_point[0])/UNIT_X, int(middle_point[1])/UNIT_Y, int(center_end[0])/UNIT_X, int(center_end[1])/UNIT_Y), fill=(255,0,0), width=3)
            ...
            def get_drawings():
                ...
                for path in draws:
                    for item in path["items"]:  # these are the draw commands
                        ...
                        # ------------------------------------
                        if item[0] == "l":  # line
                            lines.append([int(item[1].x), int(item[1].y), int(item[2].x), int(item[2].y)])
                        elif item[0] == "re":  # rectangle
                            lines.append([int(item[1].x0), int((item[1].y1 - item[1].height))])
    ...
    def set_features(self, g, bboxs, texts, size, norm = True, embedder = 'sci'):
        ...
        Args:
            ...
            embedder : (str, optional)
                If 'sci' it use en_core_sci_lg model (default).
                If 'web' it use en_core_web_lg model.
        """
        ...
        maxw, maxh = 0, 0
        if embedder == 'sci':
            spacy_embedder = spacy.load("en_core_sci_lg")
        elif embedder == 'web':
    ...
    def remove_islands(self, g):
        ...
        matrix = dgl.khop_adj(g, khop)
        for n in range(0, g.num_nodes()):
            if labels[n] != 0: continue
            isIsland = True
            for m in range(0, g.num_nodes()):
                if matrix[m, n] != 0 and m != n and labels[m] != labels[n]:
                        isIsland = False
            if isIsland:
                remove.append(n)
        g.remove_nodes(torch.tensor(remove, dtype=torch.int32))
        return remove
